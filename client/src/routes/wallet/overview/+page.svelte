...
<div class="mt-2 flex h-30 lg:h-40 overflow-x-scroll">
  {#each links as { title, icon, link } (link)}
    <a
      href={resolve(link)}
      class="m-1 lg:m-3 flex aspect-square flex-col items-center justify-center rounded-xl border border-black p-2 text-center shadow-md transition hover:border-teal-500 dark:border-white dark:bg-[rgb(35,35,35)] dark:hover:border-teal-400"
    >
